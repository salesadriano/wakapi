...
description: Reviews and authors Cloudflare Workers code against production best practices. Load only for Cloudflare Workers tasks such as writing new Workers, reviewing Worker code, configuring wrangler.jsonc, or checking Workers anti-patterns (streaming, floating promises, global state, secrets, bindings, observability). Not a generic serverless skill; it biases towards retrieval from Cloudflare docs over pre-trained knowledge.
---
