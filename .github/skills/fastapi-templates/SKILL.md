...
description: Bootstrap production-ready FastAPI projects with recommended structure, dependency injection, and deployment-safe defaults. Use when starting a new FastAPI application or reorganizing a service skeleton. Prefer fastapi-expert for feature implementation inside an existing codebase.
---
...
Use this skill for scaffolding and baseline structure. For endpoint-by-endpoint implementation in an existing service, prefer `fastapi-expert`; for async bottlenecks or concurrency reviews, prefer `fastapi-async-patterns`.
...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str
    SECRET_KEY: str  # Required from environment or secret manager; never hardcode.
