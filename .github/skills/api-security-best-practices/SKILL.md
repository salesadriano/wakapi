...
> **Scope boundary:** Use this skill for API-specific authentication, authorization, schema validation, quotas, throttling, token handling, and OWASP API risks across REST, GraphQL, and WebSockets. For browser-facing CSP, cookies, CSRF, and generic security headers, use [`security-best-practices`](../security-best-practices/SKILL.md). For framework-specific hardening, prefer the corresponding framework skill.
...
For browser-facing CSP with inline assets, generate a fresh nonce per request and follow [`security-best-practices`](../security-best-practices/SKILL.md). Do not use static nonce values.
\`\`\`
...
- [Auth Patterns Quick Reference](references/auth-patterns.md) — JWT, API keys, OAuth/PKCE, ABAC, tiered rate limiting
