...
```html
<!-- Nonces shown below are placeholders for per-request values. Never reuse a fixed nonce across responses. -->
...
Content-Security-Policy: 
  default-src 'self';
  script-src 'self' 'nonce-{{RANDOM_PER_REQUEST}}' https://trusted.com;
  style-src 'self' 'nonce-{{RANDOM_PER_REQUEST}}';
  img-src 'self' data: https:;
  connect-src 'self' https://api.example.com;
...
# Legacy-only: avoid relying on X-XSS-Protection as a modern baseline
# Prefer CSP and framework output escaping instead
...
- [Core Engineering Principles](references/principles.md) — SOLID, DRY/WET/DAMP, YAGNI, KISS, CQS, Law of Demeter, code review checklist
