...
description: Django-specific security hardening for authentication, permissions, CSRF, XSS, secure settings, and deployment reviews. Use when the main task is hardening or auditing a Django app, not when implementing generic Django features or routine feature work.
metadata:
    origin: ECC
---
...
- Hardening Django authentication and authorization
- Implementing user permissions and roles
- Configuring production security settings
- Reviewing Django application for security issues
- Deploying Django applications to production

> **Scope boundary:** This skill covers Django-specific security patterns (ORM, middleware, settings, permissions). Use [`django-expert`](../django-expert/SKILL.md) for routine Django feature implementation, models, views, and standard authentication flows. For generic web hardening applicable to any stack — HTTPS headers, CSP, CORS, secret management — also apply [`security-best-practices`](../security-best-practices/SKILL.md). For API authentication, authorization, quotas, and rate limiting, see [`api-security-best-practices`](../api-security-best-practices/SKILL.md).
...
Modern browsers ignore `X-XSS-Protection`; prefer Django auto-escaping, safe template APIs, and a strong CSP.
...
CSRF_COOKIE_HTTPONLY = True  # If enabled, read the token from the DOM instead of the cookie in JS
...
# AJAX requests when CSRF_COOKIE_HTTPONLY = True
...
```
    ...
    If inline scripts or styles are unavoidable, generate a fresh nonce per request; never hardcode a nonce value in settings or examples.
