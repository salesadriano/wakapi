...
When you finish building something, persist reusable patterns only when the active workflow explicitly supports project memory or when the user asks for it. Do not prompt the user to save or copy files as a default closing step.

If the workflow supports persistence and the user wants it, write to `.interface-design/system.md`:
- Direction and feel
